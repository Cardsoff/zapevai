...
export async function getSong(id) {
  ...
  // гость открывает песню из общей базы (без служебных полей)
  if (sb) {
    const { data } = await sb
      .from("songs_public")
      .select("*")
...
export async function addSong({ title, artist, lyrics, force = false }) {
  ...
  // тарифный лимит (skipLimit — для переноса данных гостя)
  if (!force) {
...
export async function addToLibrary(songId, skipLimit = false) {
  const user = await cloudUser();
  if (!user) return {};
  if (!skipLimit) {
    const plan = await getPlan();
...
// ---------- СТАТИСТИКА СЛОВ (умные пропуски) ----------
export async function getWordStats(songId) {
  const user = await cloudUser();
  if (user) {
    const sb = getSupabase();
    const { data } = await sb
      .from("word_stats")
      .select("word_index, hits, misses, runs_since_hidden")
      .eq("user_id", user.id)
      .eq("song_id", songId);
    const map = {};
    (data || []).forEach((r) => {
      map[r.word_index] = { h: r.hits, m: r.misses, r: r.runs_since_hidden };
    });
    return map;
  }
  const all = lsGet(K_WORDS, {});
  return all[songId] || {};
}

export async function saveWordStatsRun(songId, hiddenArr, openedArr) {
  const user = await cloudUser();
  if (user) {
    const sb = getSupabase();
    await sb.rpc("save_word_stats", {
      p_song: songId,
      p_hidden: hiddenArr,
      p_opened: openedArr,
    });
  ...
  const all = lsGet(K_WORDS, {});
  const m = all[songId] || {};
  Object.values(m).forEach((st) => (st.r = (st.r || 0) + 1));
  const opened = new Set(openedArr);
  hiddenArr.forEach((i) => {
    const st = m[i] || { h: 0, m: 0, r: 0 };
    if (opened.has(i)) st.m += 1;
    else st.h += 1;
    st.r = 0;
    m[i] = st;
  });
...
// Живой счётчик песен в общей базе (доступно и гостям)
export async function countBaseSongs() {
  const sb = getSupabase();
  if (!sb) return null;
  try {
    const user = await cloudUser();
    const src = user
      ? sb.from("songs").select("id", { count: "exact", head: true }).is("parent_id", null)
      : sb.from("songs_public").select("id", { count: "exact", head: true });
...
export async function listBaseSongs(limit = 8) {
  ...
  if (!sb) return [];
  const user = await cloudUser();
  const src = user ? sb.from("songs").select("id, title, artist").is("parent_id", null) : sb.from("songs_public").select("id, title, artist");
  const { data } = await src.limit(50);
  const arr = data || [];
...
export async function migrateGuestData() {
  ...
  // статистика умных пропусков (по локальным id -> облачные)
  try {
    const wordAll = lsGet(K_WORDS, {});
    for (const [localId, stats] of Object.entries(wordAll)) {
      const cid = idMap[localId];
      if (!cid) continue;
      const hidden = Object.keys(stats).map(Number);
      const opened = hidden.filter((i) => (stats[i]?.m || 0) > 0);
      if (hidden.length) await saveWordStatsRun(cid, hidden, opened);
    }
    lsSet(K_WORDS, {});
  } catch {}
...
export async function adminEvents(days = 7) {
  const sb = getSupabase();
  const { data } = await sb.rpc("admin_events", { days });
  return data || [];
}
...
// Проверка пачки на дубли перед загрузкой: exact (тот же текст) / name (то же название+исполнитель) / new
export async function adminCheckBulk(items) {
  const user = await cloudUser();
  if (!user || !ADMIN_EMAILS.includes(user.email)) return null;
  const sb = getSupabase();
  const { data: all } = await sb.from("songs").select("id, title, artist, hash");
  const norm = (s) =>
    (s || "")
      .toLowerCase()
      .replace(/ё/g, "е")
  ...
  return items.map((it) => {
    ...
    const exact = (all || []).find((s) => s.hash === hash);
    if (exact)
      return { ...it, check: "exact", existingId: exact.id, existingTitle: exact.title };
    const byName = (all || []).find(
      (s) => norm(s.title) === norm(it.title) && norm(s.artist) === norm(it.artist)
    );
    if (byName)
      return { ...it, check: "name", existingId: byName.id, existingTitle: byName.title };
    return { ...it, check: "new" };
  });
}

// Пачковая загрузка; item.action: "skip" | "update" (нужен existingId) | "new"
export async function adminBulkAddSongs(items) {
  ...
  for (const it of items) {
    ...
    try {
      if (it.action === "skip") {
        results.push({ title: it.title, status: "duplicate", detail: it.existingTitle || "" });
        continue;
      }
      if (it.action === "update" && it.existingId) {
        ...
        else results.push({ title: it.title, status: "updated" });
        continue;
      }
      const { data: existing } = await sb
        .from("songs")
