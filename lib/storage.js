...
import { nextSrs, todayStr, calcStreak, isDue } from "./srs";
...
// Добавление песни с проверкой на дубль (по хэшу текста)
export async function addSong({ title, artist, lyrics }) {
...
export async function getProgress(songId) {
  ...
  if (user) {
    const sb = getSupabase();
    const [{ data: rows }, { data: srsRow }] = await Promise.all([
      sb
        .from("progress")
    ...
    const p = { cloze: {}, letters: null, relay: null, srs: null };
    (rows || []).forEach((r) => {
  ...
  return all[songId] || { cloze: {}, letters: null, relay: null, srs: null };
}
...
export async function getStats() {
  ...
  if (user) {
    const sb = getSupabase();
    const [{ data: days }, { data: lib }, { data: prog }] = await Promise.all([
      sb.from("activity").select("day").eq("user_id", user.id),
      sb.from("user_songs").select("song_id").eq("user_id", user.id),
      sb
        .from("progress")
        .select("song_id, mode, level, best_score")
        .eq("user_id", user.id)
        .eq("mode", "cloze")
        .eq("level", 5)
        .gte("best_score", 90),
    ...
    return {
      streak: calcStreak((days || []).map((d) => d.day)),
      totalSongs: (lib || []).length,
      mastered: new Set((prog || []).map((p) => p.song_id)).size,
    };
  }
  const songs = lsGet(K_SONGS, []);
  const prog = lsGet(K_PROG, {});
  const mastered = songs.filter((s) => (prog[s.id]?.cloze?.[5] || 0) >= 90);
  return {
