...
// Свежие песни общей базы (для подсказок новичку)
...
// ---------- ЗАМЕЧАНИЯ К ПЕСНЯМ ----------
export async function reportSong(songId, comment) {
  const user = await cloudUser();
...
// Правка текста песни (автор или админ). Хэш пересчитывается.
export async function updateSongLyrics(songId, lyrics) {
  const user = await cloudUser();
  const hash = hashLyrics(lyrics);
  if (user) {
    const sb = getSupabase();
    const { error } = await sb.rpc("update_song_lyrics", {
      target: songId,
      new_lyrics: lyrics.trim(),
      new_hash: hash,
    });
    return { error: error?.message };
  }
...
// Привязать приглашение, если в localStorage лежит реф-код
